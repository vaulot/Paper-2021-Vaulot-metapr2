...
# Replace by the directory where the script and parameters reside
...
dataset_id <- 1 # CHANGE
dataset_path <- "PATH_DATA_SET" # CHANGE to dataset path on your server our computer. Fastq files should be in DATA_SET_PATH/fastq

pr2_file <- "PATH_PR2_DATABASE" # CHANGE
...
# primer sets (TAReuk454FWD1	CCAGCASCYGCGGTAATTCC	V4R	ACTTTCGTTCTTGAT) 
...
path_scratch <- function(file_name) str_c(dataset_path, file_name)
...
if (remove_primers){
  ...
  if (do_taxo) {
    ...
    # TO ADAPT - MUCH BETTER THAN MY PREVIOUS METHOD
    # # Slice file by 1000
    # slice_size = 1000
    # 
    # seq <- fasta_read(seq_file, compress=TRUE)
    # 
    # seq_number = nrow(seq)
    # 
    # seq <- seq %>%
    #   mutate(slice =ggplot2::cut_width(1:seq_number,labels=FALSE, width=slice_size, boundary = 1) )
    # 
    # slice_number = max(pull(seq, slice))
    # 
    # 
    ...
    #   file_name = stringr::str_replace(seq_file, ".fasta.gz", stringr::str_c("_",sprintf("%03d", i),".fasta.gz"))
    #   fasta_write(seq_one, file_name, compress = TRUE)
    #   dada2_assign(file_name,pr2_file)
    # }
    # 
